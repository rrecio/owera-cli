...
"""
        ...
        for line in lines:
            ...
            elif in_code and (line.startswith(('#', 'return', 'if ', 'for ', 'while ', 'try ', 'except ', 'with ', 'class ', '@login_required')) or any(line.startswith(prefix) for prefix in ('items =', 'courses =', 'user =', 'enrollment =')) or line.endswith(':')):
                code_lines.append(line)
            # Stop capturing if we hit a non-code line after starting
            elif in_code and not (line.startswith(('#', 'return', 'if ', 'for ', 'while ', 'try ', 'except ', 'with ', 'class ', '@login_required')) or line.endswith(':')):
                break
        if code_lines:
...
class Developer(Agent):
    ...
    def generate_prompt(self, task, project):
        ...
        if task.type == "implement":
            ...
            return (f"Generate a Flask route for the feature '{task.feature.name}' in {tech_stack['backend']}. "
    ...
    def plan(self, project):
        ...
        for feature in project.features:
            if not feature.has_design and not any(t.feature == feature and t.type == "design" for t in project.tasks):
                ...
                logging.debug(f"Assigned design task for feature: {feature.name}")
                ...
                project.tasks.append(task)
                logging.debug(f"Assigned implement task for feature: {feature.name}")
                ...
                project.tasks.append(task)
                logging.debug(f"Assigned test task for feature: {feature.name}")
                ...
                project.tasks.append(task)
                logging.debug(f"Assigned review task for feature: {feature.name}")
